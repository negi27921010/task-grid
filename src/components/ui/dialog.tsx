...
export function Content({
  ...
  return (
    <DialogPrimitive.Portal>
      ...
      >
        {children}
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm p-1 text-gray-400 opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          <X className="h-4 w-4" />
...
export function Description({
  ...
  return (
    <DialogPrimitive.Description
      className={cn('mt-1.5 text-sm text-gray-500', className)}
      {...props}
    />
  );
}
